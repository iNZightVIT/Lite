...
collapse.sidebar.panel <- function(data.set) {
  ...
  list(
    selectInput("select.collapse.column", "Choose a variable",
      choices = c("", choices1), multiple = F, selectize = F,
      selected = 1
    ),
    selectInput("select.collapse.item", "Choose two or more levels",
      choices = choices2, multiple = T, selectize = F, size = 7
    ),
    textInput("collapse_variable_newname",
      label = "New variable name",
      value = ""
    ),
    textInput("collapse_level_newname",
      label = "Collapsed level name",
      value = ""
    ),
    ...
    br(), br(),
    help.display(
      "Collapse Levels", "collapse_levels",
      "panels/E1_CategoricalVariables/5_collapse.levels.help.md"
    ),
    br(), HTML("")
  )
}
...
reorder.sidebar.panel <- function(data.set) {
  ...
  list(
    selectInput("select.reorder.column", "Variable to reorder",
      choices = c("", choices1), multiple = F, selectize = F
    ),
    textInput("recorder_variable_newname",
      label = "New variable name",
      value = ""
    ),
    selectInput("recorder_sort_levels", "Sort levels",
      choices = c("by frequency", "manually"), multiple = F,
      selectize = F, selected = 1
    ),
    conditionalPanel(
    ...
    br(), br(),
    help.display("Reorder Levels", "reorder_levels", "panels/E1_CategoricalVariables/4_reorder.levels.help.md"),
    br(), HTML("")
  )
}
...
combine.sidebar.panel <- function(data.set) {
  ...
  list(
    ...
    #         the data."),
    selectInput("select.combine.columns",
      "Choose 2 or more variables you want to combine",
    ...
    br(), br(),
    help.display(
      "Combine Levels", "combine_levels",
      "panels/E1_CategoricalVariables/7_combine.levels.help.md"
    ),
    br(), HTML("")
  )
}
...
rename.levels.sidebar.panel <- function(data.set) {
  ...
  list(
    ...
    br(), br(),
    help.display(
      "Rename Levels", "rename_levels",
      "panels/E1_CategoricalVariables/6_rename.levels.help.md"
    ),
    br(), HTML("")
  )
}
