...
generated will either be a box plot (for numeric variables) or a histogram (for 
...
second variable depends on the type of variable chosen - read the introduction 
above.
...
4. **Advanced Options** (*advanced users only*) allows you to customize the 
visual appearance or add inferential markups on top of the plot.
...
disappear when as inference parameter mean is selected. The boxplot shows the 
...
be added to the plot. The colour of the line can be specified. Note, if the line 
falls outside the plot it will not be displayed.
...
axis can be visua;ized by adding rugs to the plot. For more information see the 
...
- **Identify points** A feature not yet implemented which lets the user select 
points by ranges of x or y axis and label them. 
