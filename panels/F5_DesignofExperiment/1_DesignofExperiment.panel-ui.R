  ...
  sidebarPanelUI <- list(
    ...
    tabsetPanel(
      ...
      tabPanel(
        ...
        fluidRow(
          column(12, hr()),
          tags$div(column(12, radioButtons(
            inputId = "fit_design", label = "Select design",
            choices = c(
              "Standard design" = 1,
              "Customized design" = 2
            ), inline = T,
        ...
        ## select model to fit
        conditionalPanel(
          condition = "input.fit_design == 1",
          fluidRow(column(12, selectInput("model_design",
            label = "",
            choices = c(
              "One-way anova (no Blocking)" = 1,
        ...
        conditionalPanel(
          ...
          checkboxInput(
            inputId = "select_random_effect",
            label = "Structural component (random effect)", value = FALSE
          ),
          conditionalPanel(
            condition = "input.select_random_effect == 1",
...
mixedModel.mainPanel <- function() {
  mainPanelUI <- list(
    div(
      ...
      conditionalPanel(
        "input.Doe_type == 'Interaction Plot'",
        tabsetPanel(
          type = "pills",
          tabPanel(
            "Interaction Plot",
