get.create.variables.panel <- function(data.set) {
  if (is.null(data.set)) {
    sidebarLayout(
      sidebarPanel(
        help.display(
          ...
          "panels/E4_CreateVariables/3_create.variables.help.md"
        ),
        br(), HTML("")
      ),
    ...
    sidebarLayout(
      sidebarPanel(
        helpText("Select the columns to be included in the
        ...
        div(fixedRow(
          ...
          column(width = 1, "="),
          column(
            width = 7, "The expression to be evaluated",
            br(), br(),
            verbatimTextOutput("create.variables.expression")
          )
        )),
        actionButton(
          inputId = "create.variables.submit",
