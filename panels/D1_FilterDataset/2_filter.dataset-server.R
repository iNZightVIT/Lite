    ...
    if(!is.null(input$filter_data_perform)&&input$filter_data_perform>0){
      if(input$select_filter%in%"levels of categorical variable"){
        if(!is.null(input$select_categorical1)&&!input$select_categorical1%in%""){
          ...
          temp = iNZightTools::filterLevels(get.data.set(), var = input$select_categorical1, levels = input$levels1)
          if(!is.null(temp)){
            ## save code 
            code.save$dataname = paste(code.save$name, "filtered", sep = ".")
            code = tidy_assign_pipe(gsub("get.data.set\\()", code.save$name, iNZightTools::code(temp)))
        ...
        if(!input$select_numeric1%in%""&!input$select_operation1%in%""&all(is.convertable.numeric(input$numeric_input1))){
          ...
          #  indexes.keep = which((get.data.set()[,which(colnames(get.data.set())%in%input$select_numeric1)]!=as.numeric(input$numeric_input1)))
          #}
          temp = iNZightTools::filterNumeric(get.data.set(), var = input$select_numeric1, op = input$select_operation1,
        ...
        if(all(is.convertable.numeric(input$numeric_input2))&&
          ...
          temp = iNZightTools::filterRandom(get.data.set(), n = as.numeric(input$numeric_input3), sample_size = as.numeric(input$numeric_input2))
          if(!is.null(temp)){
            ## save code 
            code.save$dataname = paste(code.save$name, "filtered", sep = ".")
            code = tidy_assign_pipe(gsub("get.data.set\\()", code.save$name, iNZightTools::code(temp)))
