...
separate.columns.sidebar = function(){
  list(h5(strong("Separate columns")),
       
       selectInput(inputId="select_separate_mode",
                   label = "Select separate mode",
                   choices = c("", "Separate a column into several columns", "Separate a column to make several rows"),
                   selectize = FALSE,
                   multiple = F),
       
       selectInput(inputId="select_column_to_separate",
       ...
       fixedRow(column(3, actionButton("preview_separatecolumns_button", "Preview",
